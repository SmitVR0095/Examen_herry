...
def Exponente(numero, exponente):
    ...
    '''
    #Tu código aca:
    if(exponente==0): #Si el exponente es 0 devuelve 1
        return 1
    elif(exponente==1): #Si el exponente es 1 devuelve en número ingresado
        return numero
    elif(numero==0): #Si la base (numero=0) devuelve 0
        return 0
    else: #caso contrario hacemos un bucle
...
def Es_primo(numero):
    '''
    Esta funcion me devuelve un valor booleano es decir si en numero ingresado es primo devuelve TRUE caso contraio devuelve FALSE
    Ejemplo:
       Es_primo(7) debe retornar TRUE
       Es_primo(6) debe retornar FALSE
    '''
    primo=True
...
def ListaRepetidos(lista):
    ...
    #Tu código aca:
    repeticiones={}
    for i in lista: #Hacemos que la variable iterador "i" recorra toda la lista
        if i in repeticiones: #Si "i" pertenece al diccionario entonces 
            repeticiones[i]+=1 #se agraga un elemento más a la llave "i"
        else: #Caso contrario
            repeticiones[i]=1 #Inicializamos la llave "i" del diccionario con 
    return repeticiones
